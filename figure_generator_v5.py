...
###################################### 2 ######################################
def plot(set_of_scans, type_of_scan, cmap='jet_r',with_fit=False):
...
def correlation_pressure():
    ...
    subplot_correlation_pressure('Alrp',ax1,ax2)
    subplot_correlation_pressure('AlLi',ax3,ax4)
...
def correlation_laser():
    ...
    subplot_correlation_laser('AlLi',ax1,ax2)
    subplot_correlation_laser('Alrp',ax3,ax4)
    subplot_correlation_laser('Alrp2',ax3,ax4)
    ax1.set_xlim([20,115])
    ax2.set_xlim([20,115])
    ax3.set_xlim([-11,50])
    ax4.set_xlim([-11,50])
...
def subplot_correlation_laser(set_of_scans,ax1,ax2):
    ...
    ax2.errorbar(I,s2,yerr=3*np.sqrt(yerr2),
        marker='x',markersize=6,color='k',ls='None',ecolor='0.7')
    #factor 3: a rough increase of the errorbar making it more representative
    plt.setp(ax1.get_xticklabels(),visible=False)
    ax1.yaxis.set_ticks_position("right")
    ax2.yaxis.set_ticks_position("right")
...
def power(): # Conversionn between the data value and the actual laser power
...
def f(t,a0,a1,s1,a2,s2): # The fitting function
...
#plot(Set('AlLi'),'XANES')
#plot(Set('AlLi'),'QE',cmap='bone_r',with_fit=True)
